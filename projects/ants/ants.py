...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        #if exit isnt none, then the entrance is itself
        if exit:
            exit.entrance = self 
            #go through unlocking tests 
        # END Problem 2
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                #must bind the ant to the container ant to ensure the container
                #ant takes the damage
                # the place only knows about 1 ant which is the container ant. 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #if count is at most 3, return bee_selector
        #if count is at least 5 return bee_Selector
        counter = 1
        if self.place.bees:
            if self.min_range != 5 or self.max_range != 3:
                return bee_selector(self.place.bees)

        else:
            while not self.place.entrance.is_hive():
                if self.place.entrance.bees and counter <= self.max_range and counter >= self.min_range:      #if True (if there is a bee)
                    return bee_selector(self.place.entrance.bees)
                self.place.entrance = self.place.entrance.entrance 
                counter += 1
        return None 
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            if self.health <= amount:
                #if the ant would die it would deal extra damage
    ...
    def action(self, gamestate):
        if self.chewing != 0:
            ...
            if self.place.bees:
                ...
                #drop the health of the selected bee to 0
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        #do what your contained ant does
