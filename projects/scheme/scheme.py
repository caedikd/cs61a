...
def eval_all(expressions, env):
    ...
    #to not edit expressions
    expression = expressions
    while expression.rest is not nil:
        scheme_eval(expression.first, env)
        expression = expression.rest
    return scheme_eval(expression.first, env)
    # END PROBLEM 7
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        #this is the format for adding values to dictionaires?
        # END PROBLEM 2

    def lookup(self, symbol):
        """Return the value bound to SYMBOL. Errors if SYMBOL is not found."""
        # BEGIN PROBLEM 2
        #takes a symbol and returns the value
        #bound to that name in the first Frame 
        #that the name is found, in the current env
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        arguments_list = []
        # BEGIN PROBLEM 3
        #convert scheme list into a python list
        while args is not nil:
            arguments_list.append(args.first)
            args = args.rest

        #if self.use_env is true then add the current env
        #as the last argument to the python list
        if self.use_env:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        #we are going to be using the environment where
        #the lambda is defined not where it is called
        #create a new frame instance then using the new 
        #frame instance we can bind the appropriate argument
        #values and then return that frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        values = scheme_eval(expressions.rest.first, env)
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        #correctly handles the shorthand procedure 
        #definition form above. and can handle multi-
        #expression bodies
        name = target.first
        formals = target.rest
...
def do_lambda_form(expressions, env):
    ...
    #expressions is everything excluding the lambda
    body = expressions.rest
    new = LambdaProcedure(formals, body, env)
    return new
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    #returns a child frame of env that binds the symbol in each element
    #of bindings to the value of its corresponding expression
    names, values = nil, nil
    while bindings is not nil:
        symbol = bindings.first
        validate_form(symbol, 2, 2)
        name = symbol.first
        value = scheme_eval(symbol.rest.first, env)
