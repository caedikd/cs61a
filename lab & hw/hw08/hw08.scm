...
;accumulate combines the first n natural
;nums according to combiner star, n, term
  (define (helper combiner n term so-far)
    (if (= n 0) so-far
      (helper combiner (- n 1) term (combiner (term n) so-far))
    ))
