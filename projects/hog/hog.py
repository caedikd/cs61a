...
def piggy_points(score):
    ...
    minimum = score_squared % 10

    while score_squared > 0:
            num = score_squared % 10
            if num < minimum:
                minimum = num
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls == 0:
        goal = piggy_points(opponent_score)
    else:
        goal = roll_dice(num_rolls, dice)
    return goal
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    roll = roll_dice(trials_count, dice)
    if (make_averaged(roll_dice(i, dice),)
    sum = make_averaged(roll,trials_count)
    return print(sum)
